...
function Write-Section { Write-Host "`n$('=' * 70)`n  $args`n$('=' * 70)" -ForegroundColor Yellow }
...
function Get-AuthToken {
    ...
    Write-Info "Authenticating as $($User.username)..."
    ...
    if ($response -and $response.data -and $response.data.token) {
        Write-Success "Authentication successful as $($User.username)"
    ...
    elseif ($response -and $response.token) {
        ...
        Write-Failure "Authentication failed for $($User.username)"
        return $null
    }
}
...
function Test-DashboardEndpoints {
    ...
    Invoke-ApiRequest -Method GET -Endpoint "/dashboard/admin" -Token $Token
...
function Test-AssessmentEndpoints {
    ...
    Invoke-ApiRequest -Method GET -Endpoint "/assessments" -Token $Token
...
function Show-Summary {
    ...
    Write-Host "â„¹ Detailed results saved to: $resultFile" -ForegroundColor Cyan
