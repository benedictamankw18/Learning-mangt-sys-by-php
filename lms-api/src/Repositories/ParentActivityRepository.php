...
class ParentActivityRepository extends BaseRepository
{
    protected $table = 'parent_activity';
    protected $primaryKey = 'activity_id';

    /**
     * Log a new parent activity.
     */
    ...
    public function getAllByInstitution(int $institutionId, array $filters = [], int $limit = 50, int $offset = 0): array
    {
        $sql = "SELECT pa.*,
                ...
                FROM {$this->table} pa
                LEFT JOIN users u ON pa.performed_by = u.user_id
                WHERE pa.institution_id = :institution_id";
        $params = [':institution_id' => $institutionId];

        if (!empty($filters['performed_by'])) {
            $sql .= " AND pa.performed_by = :performed_by";
            $params[':performed_by'] = (int) $filters['performed_by'];
        }

        if (!empty($filters['activity_type'])) {
            $sql .= " AND pa.activity_type = :activity_type";
            $params[':activity_type'] = $filters['activity_type'];
        }

        if (!empty($filters['entity_type'])) {
            $sql .= " AND pa.entity_type = :entity_type";
            $params[':entity_type'] = $filters['entity_type'];
        }

        if (!empty($filters['entity_id'])) {
            $sql .= " AND pa.entity_id = :entity_id";
        ...
        if (!empty($filters['start_date'])) {
            $sql .= " AND pa.created_at >= :start_date";
            $params[':start_date'] = $filters['start_date'];
        }

        if (!empty($filters['end_date'])) {
            $sql .= " AND pa.created_at <= :end_date";
    ...
    public function getByType(int $institutionId, string $type, int $limit = 50, int $offset = 0): array
    {
        $stmt = $this->db->prepare("
            ...
            LEFT JOIN users u ON pa.performed_by = u.user_id
            WHERE pa.institution_id = :institution_id
              AND pa.activity_type = :activity_type
            ORDER BY pa.created_at DESC
            LIMIT :limit OFFSET :offset
        ");
        $stmt->bindValue(':institution_id', $institutionId, PDO::PARAM_INT);
