...
class TeacherActivityRepository extends BaseRepository
{
    protected $table = 'teacher_activity';
    ...
    public function getAllByInstitution(int $institutionId, array $filters = [], int $limit = 50, int $offset = 0): array
    {
        ...
        if (!empty($filters['performed_by'])) {
            $sql .= " AND ta.performed_by = :performed_by";
            $params[':performed_by'] = (int) $filters['performed_by'];
        }

        if (!empty($filters['activity_type'])) {
            $sql .= " AND ta.activity_type = :activity_type";
            $params[':activity_type'] = $filters['activity_type'];
        }

        if (!empty($filters['entity_type'])) {
            $sql .= " AND ta.entity_type = :entity_type";
            $params[':entity_type'] = $filters['entity_type'];
        }

        if (!empty($filters['entity_id'])) {
            $sql .= " AND ta.entity_id = :entity_id";
            $params[':entity_id'] = (int) $filters['entity_id'];
        }

        if (!empty($filters['severity'])) {
            $sql .= " AND ta.severity = :severity";
            $params[':severity'] = $filters['severity'];
        }

        if (!empty($filters['start_date'])) {
            $sql .= " AND ta.created_at >= :start_date";
            $params[':start_date'] = $filters['start_date'];
        }

        if (!empty($filters['end_date'])) {
            $sql .= " AND ta.created_at <= :end_date";
            $params[':end_date'] = $filters['end_date'] . ' 23:59:59';
        }

        $sql .= " ORDER BY ta.created_at DESC LIMIT :limit OFFSET :offset";
    ...
    public function getByType(int $institutionId, string $type, int $limit = 50, int $offset = 0): array
    {
        $stmt = $this->db->prepare("
            ...
            LEFT JOIN users u ON ta.performed_by = u.user_id
            WHERE ta.institution_id = :institution_id
              AND ta.activity_type = :activity_type
            ORDER BY ta.created_at DESC
            LIMIT :limit OFFSET :offset
        ");
        $stmt->bindValue(':institution_id', $institutionId, PDO::PARAM_INT);
