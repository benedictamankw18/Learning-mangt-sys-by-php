...
(function () {
  ...
  function init() {
    const root = el('parentAnnouncementsRoot');
  ...
  function bindEvents() {
    el('panRefreshBtn')?.addEventListener('click', loadAnnouncements);

    el('panSearchInput')?.addEventListener('input', function (event) {
      STATE.search = String(event.target.value || '').trim().toLowerCase();
      filterAndRender();
    });

    el('panPriorityFilter')?.addEventListener('change', function (event) {
  ...
  function renderStats() {
    ...
    setText('panStatTotal', total);
    setText('panStatUrgent', urgent);
    setText('panStatFiles', files);
    setText('panStatLatest', latest || '-');
  }
  ...
  function renderList() {
    ...
    list.innerHTML = STATE.filtered.map(function (item) {
      const targetLabel = item.target_role === 'parent' ? 'Parents' : (item.target_role === 'all' ? 'School' : capitalize(item.target_role));
        ...
        '<div class="ann-item-head">' +
          '<div>' +
            ...
            '<div class="ann-meta">' +
              ...
              '<span class="ann-chip ' + esc(item.target_role || 'all') + '">' + esc(targetLabel) + '</span>' +
