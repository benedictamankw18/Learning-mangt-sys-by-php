...
(function () {
  ...
  async function loadData() {
    ...
    try {
      ...
      const [usersResponse, roomsResponse] = await Promise.all([
        loadAllUsers(),
        ChatAPI.getRooms().catch(function () { return null; }),
      ]);
  ...
  function renderRooms() {
    ...
    DOM.roomList.innerHTML = rooms.map(function (room) {
      ...
      return '<div class="messages-room-item ' + active + '" data-room-uuid="' + esc(room.uuid || '') + '">'
        + '<div class="messages-avatar">' + esc(initial) + '</div>'
  ...
  function scrollToReadPosition(room) {
    ...
    const target = lastReadMessageId > 0
  ...
  async function markSelectedRoomRead() {
    ...
    if (!lastMessage || Number(lastMessage.sender_id || 0) === Number(STATE.currentUser?.user_id || 0)) {
      return;
    }
